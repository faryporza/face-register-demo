...
export default function CheckinsPage() {
  ...
  useEffect(() => {
    const loadLogs = async () => {
      try {
        ...
        setLoading(true);
        const response = await fetch(`/api/checkin-logs?page=${page}&limit=${limit}`,
          {
            headers: {
  ...
  if (user?.type?.toLowerCase?.() !== 'admin') {
    return (
      <HomeShell user={user} active="checkins" onLogout={handleLogout}>
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 text-center text-slate-500">
          เฉพาะผู้ดูแลระบบเท่านั้นที่สามารถดูบันทึกการลงเวลาได้
        </div>
      </HomeShell>
    );
  }
    ...
    <HomeShell user={user} active="checkins" onLogout={handleLogout}>
      <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
        <div className="p-6 border-b border-slate-100 flex items-center justify-between">
          <h3 className="font-bold text-lg text-slate-800">ประวัติการลงเวลา (Admin)</h3>
        </div>
